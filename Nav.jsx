    ...
    <button onClick={toggle} aria-label="Toggle theme"
      ...
      {light ? (
        ...
        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24"
          fill="#f5c542" stroke="#f5c542" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
...
export default function Nav() {
  ...
  useEffect(() => {
    const onScroll = () => {
      ...
      navRef.current.style.borderBottomColor = window.scrollY > 40 ? border : 'transparent';
    };
    ...
    <>
      <nav ref={navRef} id="nav"
        ...
        <a href="#" onClick={(e) => { e.preventDefault(); window.scrollTo({ top: 0, behavior: 'smooth' }); }}
          ...
          <img
            src={isLight ? 'data/assets/logo.png' : 'data/assets/logo_black.png'}
            alt={nav.name}
          ...
          {nav.links.map((l) => (
            <a key={l.href} href={l.href}
              className="nav-link font-mono text-[13px] tracking-[0.16em] uppercase text-skin-text
                hover:text-skin-text/70 transition-colors duration-200">
              {l.label}
            </a>
          ))}
        </div>
